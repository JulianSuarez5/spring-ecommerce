...
public class AdminController {
    ...
    @Autowired
    [cite_start]private ProductService productService; [cite: 694]
    @Autowired
    [cite_start]private CategoryService categoryService; [cite: 695]

    @Autowired
    [cite_start]private BrandService brandService; [cite: 695]
    @Autowired
    [cite_start]private OrderService orderService; [cite: 696]

    @Autowired
    [cite_start]private UserRepository userRepository; [cite: 696]
    @Autowired
    [cite_start]private org.springframework.security.crypto.password.PasswordEncoder passwordEncoder; [cite: 697]

    @GetMapping
    public String adminDashboard(Model model, Authentication authentication) {
        [cite_start]// Verificar que el usuario tenga rol de administrador [cite: 697]
        if (authentication == null || !authentication.getAuthorities().stream()
                .anyMatch(auth -> auth.getAuthority().equals("ROLE_ADMIN"))) {
            [cite_start]return "redirect:/login?error=access_denied"; [cite: 697]
        }
